...
static const char isShowEmptyKey;
static const CGFloat dataVerticalOffsetKey;
static const char kCenterYKey;
...
- (void)setCenterY:(CGFloat)centerY{
    objc_setAssociatedObject(self, &kCenterYKey, @(centerY), OBJC_ASSOCIATION_ASSIGN);
}
...
- (void)setIsShowEmpty:(BOOL)isShowEmpty {
    ...
    self.emptyDataSetSource = self;
    self.emptyDataSetDelegate = self;
    objc_setAssociatedObject(self, &isShowEmpty, @(isShowEmpty), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (BOOL)isShowEmpty {
    id empty = objc_getAssociatedObject(self, &isShowEmptyKey);
    NSNumber *isShow = empty;
    return isShow.boolValue;
}
- (CGFloat)centerY{
    NSNumber *centerYNum = objc_getAssociatedObject(self, &kCenterYKey);
    return centerYNum.floatValue;
}
...
- (NSAttributedString *)buttonTitleForEmptyDataSet:(UIScrollView *)scrollView forState:(UIControlState)state
{
    if (self.loading) {
        ...
        return [[NSAttributedString alloc] initWithString:self.buttonText ? self.buttonText : @"" attributes:attributes];
    }
}
